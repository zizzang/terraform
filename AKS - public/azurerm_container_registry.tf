...
# AKS 클러스터에 대한 ACR Pull 권한 할당
resource "azurerm_role_assignment" "acr" {
  for_each             = azurerm_kubernetes_cluster.aks                                         # 각 AKS 클러스터에 대해 반복 수행
  principal_id         = azurerm_kubernetes_cluster.aks[each.key].kubelet_identity[0].object_id # 사용자 할당 관리 ID의 Principal ID
  role_definition_name = "AcrPull"                                                              # ACR Pull 역할 정의 이름 (컨테이너 이미지 풀링 권한)
  scope                = azurerm_container_registry.acr.id                                      # ACR 리소스 범위 지정
}
