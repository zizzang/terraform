...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  ...
  name                = "NodeRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.aks[each.key].name}"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.key].name
  description         = "Node Recording Rules Rule Group"
  rule_group_enabled  = true
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.aks.id, azurerm_kubernetes_cluster.aks[each.key].id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules_rule_group" {
  for_each            = azurerm_kubernetes_cluster.aks
  name                = "KubernetesRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.aks[each.key].name}"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.key].name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_and_kubernetes_recording_rules_rule_group_win" {
  for_each            = azurerm_kubernetes_cluster.aks
  name                = "NodeAndKubernetesRecordingRulesRuleGroup-Win-${azurerm_kubernetes_cluster.aks[each.key].name}"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.key].name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group_win" {
  for_each            = azurerm_kubernetes_cluster.aks
  name                = "NodeRecordingRulesRuleGroup-Win-${azurerm_kubernetes_cluster.aks[each.key].name}"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.key].name
  description         = "Node and Kubernetes Recording Rules Rule Group for Windows Nodes"
...
resource "azurerm_monitor_alert_prometheus_rule_group" "ux_recording_rules_rule_group" {
  for_each            = azurerm_kubernetes_cluster.aks
  name                = "UXRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.aks[each.key].name}"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.key].name
  description         = "UX Recording Rules Rule Group"
  rule_group_enabled  = true
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.aks.id, azurerm_kubernetes_cluster.aks[each.key].id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "ux_recording_windows_rules_rule_group" {
  for_each            = azurerm_kubernetes_cluster.aks
  name                = "UXRecordingRulesRuleGroup-Win-${azurerm_kubernetes_cluster.aks[each.key].name}"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
  cluster_name        = azurerm_kubernetes_cluster.aks[each.key].name
  description         = "Node Recording Rules Rule Group"
