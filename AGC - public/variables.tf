...
variable "vhub" {
  type = object({
    address_prefix = string // 가상 허브의 주소 범위
  })
  default = {
...
# Application Gateway for Container Blue 서브넷 설정
variable "subnet_alb_blue" {
  type = object({
...
# Application Gateway for Container Green 서브넷 설정
variable "subnet_alb_green" {
  type = object({
    index            = number
    naming           = string
...
variable "subnet_aks_blue" {
  ...
  default = {
    index            = 0
    naming           = "aks-node-blue"
    address_prefixes = ["10.1.3.0/24"]
  }
}

# AKS Green 노드 서브넷 설정
variable "subnet_aks_green" {
  type = object({
    index            = number
    naming           = string
    address_prefixes = list(string)
...
variable "aks" {
  type = object({
    default_node_pool = object({
      name       = string // 기본 노드 풀 이름
      node_count = number // 노드 수
      vm_size    = string // VM 크기
      min_count  = number // 최소 노드 수
      max_count  = number // 최대 노드 수
    })
    network_profile = object({
      network_plugin      = string // 네트워크 플러그인
      network_data_plane  = string // 데이터 평면
      network_plugin_mode = string // 플러그인 모드
    })
    sku_tier = string // SKU 계층
    node_pool = object({
      name      = string // 노드 풀 이름
      vm_size   = string // VM 크기
      min_count = number // 최소 노드 수
    ...
    vm_node_pool = object({
      name = string // VM 노드 풀 이름
    })
  })
  default = {
    default_node_pool = {
      name       = "agentpool"       // 기본 노드 풀 이름
      node_count = 2                 // 기본 노드 수
      vm_size    = "Standard_D2s_v4" // VM 크기
      min_count  = 1                 // 최소 노드 수
      max_count  = 5                 // 최대 노드 수
    }
    network_profile = {
      network_plugin      = "azure"   // 네트워크 플러그인
      network_data_plane  = "azure"   // 데이터 평면 활성화
      network_plugin_mode = "overlay" // 플러그인 모드 설정
    }
    sku_tier = "Standard" // SKU 계층 설정
    node_pool = {
      name      = "workload"        // 노드 풀 이름
      vm_size   = "Standard_D2s_v4" // VM 크기
      min_count = 1                 // 최소 노드 수
      max_count = 5                 // 최대 노드 수
    }
    vm_node_pool = {
      name = "vmworkload" // VM 노드 풀 이름
    }
  }
}
