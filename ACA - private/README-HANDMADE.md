...
- `variables.tf` : 리소스별 네이밍, 리전, VNet, 서브넷, Application Gateway, Cosmos DB, Key Vault, VM, MySQL, Allow IP, Storage, 태그 등 변수 정의
...
- `azurerm_application_gateway.tf` :  Blue/Green 배포 환경을 위한 Private Application Gateway, WAF 정책, 진단 로그 및 모니터링 구성
- `azurerm_container_apps.tf` : Blue/Green 배포용 컨테이너 앱 환경과 네트워크, 보안, DNS, 관리 ID, ACR 권한 부여 등 자동화 구성
...
- `azurerm_traffic_manager_profile.tf` : Traffic Manager Profile 생성, Private IP 등록, Weighted 기반 트래픽 분산 및 상태 모니터링 구성
