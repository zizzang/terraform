...
resource "azurerm_web_application_firewall_policy" "agw" {
  ...
  custom_rules {
    name      = "TrafficManagerAllow"               # 트래픽 관리자를 통한 접근을 허용하는 사용자 정의 규칙
    priority  = 1                                  # 우선순위 1
    rule_type = "MatchRule"                        # 매칭 규칙

    match_conditions {
      match_variables {
        variable_name = "RequestHeaders"           # 요청 헤더 변수 사용
        selector      = "Host"                     # Host 헤더 사용
      }

      operator           = "Contains"              # 포함 조건
      negation_condition = false                   # 부정 조건 아님
      match_values       = [azurerm_traffic_manager_profile.tm.fqdn] # 트래픽 매니저의 FQDN과 매칭
    }
  ...
  custom_rules {
    name      = "AllDeny"                         # 기본적으로 모든 접근 차단
    priority  = 100                               # 우선순위 100
    rule_type = "MatchRule"                       # 매칭 규칙

    match_conditions {
      match_variables {
        variable_name = "RemoteAddr"              # 원격 주소(IP) 변수 사용
      }

      operator           = "IPMatch"              # IP 매칭 조건
      negation_condition = true                   # 부정 조건(0.0.0.0이 아닌 경우)
      match_values       = ["0.0.0.0"]            # 0.0.0.0
    }
...
resource "azurerm_application_gateway" "agw" {
  ...
  backend_address_pool {
    ...
    fqdns = ["${azurerm_container_app.aca["${replace("${each.key}", "agw", "aca")}"].name}.${azurerm_private_dns_zone.aca["${replace("${each.key}", "agw", "aca")}"].name}"]
    # Container App 의 FQDN
  }
